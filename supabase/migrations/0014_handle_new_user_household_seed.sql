-- Extend the signup seed trigger to also create the new user's solo
-- household + owner membership row. category_rules/nw_targets/
-- account_templates seeding is unchanged from 0004 — only the household
-- inserts below are added. This is the highest-blast-radius single
-- statement in the household migration (any typo here breaks signup), so
-- it's sequenced last, on its own, once 0009-0013 are already verified.
