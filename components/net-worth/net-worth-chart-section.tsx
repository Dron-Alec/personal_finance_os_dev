...
export function NetWorthChartSection({
  ...
  // only render when every account is included.
  const chartGoalLines = allSelected ? goalLines : [];
  const chartGoalSeries = allSelected ? goalSeries : [];
  ...
  const rawData = useMemo(
    () =>
      buildNetWorthChartData(
        ...
        allSelected ? targets : [],
        allSelected ? goalCurves : [],
      ),
    [filteredSeries, allSelected, targets, goalCurves],
  );
